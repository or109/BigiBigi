...
public class KMeansCentersReducer extends
	...
	@Override
	protected void reduce(Vector key, Iterable<Vector> values,
		...
		double currK = 
				Math.round(-0.001d + ((double)vectors.size()) / 
						   Integer.parseInt(context.getConfiguration().get("total.stocks")) * 
		...
		for (int i = 0; i < currK; i++) {
			...
			Vector randomVector = vectors.get((int)Math.floor(Math.random() * vectors.size()));
		...
		while(vectors.size() > 0 &&
			  random.size() < Integer.parseInt(context.getConfiguration().get("k")) &&
			  currK > 0) {
	...
	protected void cleanup(Context context)
		...
		while(totalK < Integer.parseInt(context.getConfiguration().get("k")) &&
			...
			Centroid randomCentroid = random.get((int)Math.floor(Math.random() * random.size()));
			context.write(new Vector(randomCentroid.center), new Vector(randomCentroid.centroid));
